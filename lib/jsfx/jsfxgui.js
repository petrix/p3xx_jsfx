export default jsfxgui = function () {
	var Parameters = [];
		...
		for (var i = 0; i < len; i += 1) {
			...
			if (param.type === "range") {
				...
				for (j = 0; j < lenj; j++) {
					...
					input.type = "radio";
					if (name === param.def) input.checked = true;
	...
	this.SampleGenerators = {
		...
		Explosion: function (p) {
			...
			if (r() < 0.2) p.Slide = 0;
			if (r() < 0.33) p.RepeatSpeed = r(0.5, 0.3);
		...
		"Hit/Hurt": function (p) {
			var i = r(3) | 0;
			if (i === 2) i = 3;
			else if (i === 0) p.SquareDuty = r(0.6);
			p.Generator = gens[i];
	...
	this.play = function () {
		...
		if (typeof wave !== undefined) delete wave;
	...
	this.paramsToLibrary = function () {
		...
		var p = this.getParams(),
			...
			len = arr.length;
		for (var i = 0; i < len; i++)
			if (typeof arr[i] === "number") arr[i] = arr[i].toFixed(4);
			else if (typeof arr[i] === "string") arr[i] = '"' + arr[i] + '"';
